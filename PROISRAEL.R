...
data <- read_excel("proisrael.xlsx")
...
tokens_df <- data.frame(Combined_Text = words) %>%
  ...
  filter(!word %in% c("israel","says","palestine","â","palestinians","hamas","israelhamas","israeli", "palestinian", "gaza","israel","war"))
...
df2$title <- rep(c(1:22163),each = 1)
...
corpus <- tm_map(corpus, removeWords, c(stopwords("en"), "israel","palestine","hamas", "gaza","war","says","israeli"))  # Exclude "share"
