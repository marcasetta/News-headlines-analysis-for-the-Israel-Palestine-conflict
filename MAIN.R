...
data <- read_excel("data_complete.xlsx")
...
tokens_df <- data.frame(Combined_Text = words) %>%
  ...
  filter(!word %in% c("sas","africaa","icj","keir","starmer","sa","ïsrael","palestine","says","â","palestinians","hamas","israelhamas","israeli", "palestinian", "gaza","israel","war"))
...
unigrams_df <- data.frame(Combined_Text = words) %>%
  ...
  filter(!is.numeric(word)) %>%
  anti_join(get_stopwords(source = "smart", language = "en"), by = c("word" = "word")) %>%
  filter(!word %in% c("israel","says","palestine","â","biden","palestinians","hamas","israelhamas","israeli", "palestinian", "gaza","israel","war"))
...
corpus2 <- tm_map(corpus2, removeWords, c(stopwords('en'), c("sas","africaa","icj","keir","starmer","sa","ïsrael","palestine","says","â","palestinians","hamas","israelhamas","israeli", "palestinian", "gaza","israel","war")))  # Remove English stopwords and "share"
...
t %>% filter(correlation > .500) %>% # filters data 
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
...
corpus <- tm_map(corpus, removeWords, c(stopwords("en"), "says","sas","africaa","icj","keir","starmer","sa","plessis","dricus","says"))  # Exclude "share"
